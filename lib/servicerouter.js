...
const HTTPPlugin = require('./plugins/http-plugin');
const WebSocketPlugin = require('./plugins/websocket-plugin');
...
class ServiceRouter {
  ...
  */
  init(config, routesObj) {
    this.config = config;
    websocketPlugin.init(config);
    httpPlugin.init(config);
  ...
  */
  ...
  /**
   * @name getConfig
   * @summary return loaded server config object
  ...
  */
  routeWSMessage(ws, message) {
    ...
    if (msg.to.indexOf('[') > -1 && msg.to.indexOf(']') > -1) {
      ...
      if (toRoute.serviceName === this.serviceName) {
        this._handleRouterRequestWS(ws, toRoute, msg);
      } else {
      ...
      if (toRoute.serviceName === this.serviceName) {
        ...
        if (msg.forward) {
          if (toRoute.serviceName === 'hydra-router' && toRoute.apiRoute === '/queue') {
            msg.to = msg.fwd;
            delete msg.fwd;
            hydra.queueMessage(UMFMessage.createMessage(msg).toShort());
            return;
          }
  ...
  /**
   * @name wsAuthenticate
   * @summary authenticate a HTTP-style Basic auth string
   * @param {string} key
   * @return {promise} which resolves if authenticated, else rejects
   */
  wsAuthenticate(key) {
    return websocketPlugin.authenticate(key);
  }
      ...
      if (data.headers) {
        ...
        if (ct && ct.indexOf('json') > -1) {
          if (data.headers['cache-control']) {
            headers['cache-control'] = data.headers['cache-control'];
          }
          delete data.headers;
  ...
  */
  _handleRouterRequest(urlData, matchResult, request, response) {
    ...
    } else if (matchResult.pattern === '/v1/router/stream/:name') {
      this._handleStreamMessage(request, response);
  ...
  * @return {undefined}
  */
  _handleRouterRequestWS(ws, route, msg) {
    let err = false;
  ...
  */
  _handleSendMessage(request, response) {
    ...
    request.on('end', async() => {
      try {
        umf = UMFMessage.createMessage(Utils.safeJSONParse(umf));
        let toRoute = UMFMessage.parseRoute(umf.to);
        this.httpStats.log(toRoute.serviceName);
      } catch (err) {
        this.log(FATAL, `HR: ${err.message}`);
        this.log(FATAL, err);
        serverResponse.sendInvalidRequest(response);
  ...
  * @summary Route incoming HTTP UMF message to a named channel stream
  * @private
  * @param {object} request - Node HTTP request object
  * @param {object} response - Node HTTP response object
  * @return {undefined}
  */
  _handleStreamMessage(request, response) {
    let umf = '';
    request.on('data', (data) => {
      umf += data;
    });
    request.on('end', async() => {
      try {
        umf = UMFMessage.createMessage(Utils.safeJSONParse(umf)).toShort();
      } catch (err) {
        this.log(FATAL, `HR: ${err.message}`);
        this.log(FATAL, err);
        serverResponse.sendInvalidRequest(response);
      ...
      const channelName = segs[4];
      const redisClient = hydra.getClonedRedisClient();
      redisClient.xadd(`hydra:stream:${channelName}`, '*', 'message', Utils.safeJSONStringify(umf), (err, result) => {
        serverResponse.sendOk(response, {
          result: {
            id: result
          }
